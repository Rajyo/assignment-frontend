...
export default function StickyCursor({
  ...
  const rotate = (distance = { x: 0, y: 0 }) => {
    const angle = Math.atan2(distance.y, distance.x);
    animate(cursor.current, { rotate: `${angle}rad` }, { duration: 0 });
  };

  const manageMouseMove = (e: MouseEvent) => {
    if (!stickyElement) return; // add a null check
  ...
  return (
    ...
    >
      {isHovered && title !== "" ? (
        <div className="absolute text-xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white text-black rounded-full pointer-events-none ">
          {title === "play" ? <Play className="w-10 h-10" /> : title}
        </div>
      ) : null}
    </motion.div>
  );
